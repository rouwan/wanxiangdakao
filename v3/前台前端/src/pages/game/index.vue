    ...
    initSocket(_type) {
      ...
      this.fly
        ...
        .then(res => {
          ...
          let url = "";
          if (this.EVN == "Dev") {
            let code = typeof res.data === 'string' ? JSON.parse(res.data).Result.code : (res.data.Result ? res.data.Result.code : '');
            if (!code) {
              // fallback: get wsLogin token directly
              let raw = typeof res.data === 'string' ? JSON.parse(res.data) : res.data;
              code = (raw.Result && raw.Result.code) ? raw.Result.code : 'dev_fallback';
            }
            console.log('[GAME] Dev mode, wsLogin code:', code);
            url = "ws://127.0.0.1:8008/vientianetest/wsRank?code=" + code + "&&type=" + (this.isRank ? 1 : _type);
          } else {
            let code = typeof res.data === 'string' ? JSON.parse(res.data).Result.code : res.data.Result.code;
            url = "wss://ggbsq.xingpanwang.com/vientianetest/wsRank?code=" + code + "&&type=" + (this.isRank ? 1 : _type);
          }
