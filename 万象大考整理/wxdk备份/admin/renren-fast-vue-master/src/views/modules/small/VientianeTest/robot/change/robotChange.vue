  ...
  <div class="mod-log">
    ...
    <el-table
      ...
      <el-table-column
        ...
        label="行为类型">
        <template slot-scope="scope"> 
          <span>{{renderType(scope.row.actionType)}}</span>
        </template>  
      </el-table-column>
       <el-table-column
        prop="value"
...
<script>
  ...
  import AddModules from './robotChangeAdd'
  import UpdateModules from './robotChangeUpdate' 
  ...
  export default {
    ...
    methods: {
      ...
      deleteHandle(id){
         this.$confirm('此操作将永久删除该数据, 是否继续?', '提示', {
          ...
          type: 'warning'
           ...
           this.$http({
              url: this.$http.adornUrl('/smallVientianeTest/robotChange/delete'),
              method: 'get',
      ...
      getDataList (type) {
        ...
        this.$http({
          url: this.$http.adornUrl('/smallVientianeTest/robotChange/getPageList'),
          method: 'get',
          params: this.$http.adornParams({
            'type': this.type,
