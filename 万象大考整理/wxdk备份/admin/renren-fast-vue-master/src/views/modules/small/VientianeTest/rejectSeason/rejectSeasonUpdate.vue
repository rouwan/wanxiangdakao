  ...
  data() {
    return {
      ...
      dataRule: {
        ...
        season: [{ required: true, message: "理由不能为空", trigger: "blur" }],
        description: [
          { required: true, message: "说明不能为空", trigger: "blur" }
        ],
        advice: [{ required: true, message: "建议不能为空", trigger: "blur" }]
      }
    };
  },
  created() {
        ...
        if (this.basicData.id) {
          this.$http({
            url: this.$http.adornUrl(
              `/smallVientianeTest/rejectSeason/getRejectSeasonById`
            ),
            method: "get",
            ...
            if (data && data.Tag === 1) {
              ...
              this.basicData.season = data.Result.season;
              this.basicData.advice = data.Result.advice;
    ...
    dataFormSubmit() {
      ...
      this.$refs["basicData"].validate(valid => {
        if (valid) {
          ...
          var thiz = this;
          this.$http({
            url: this.$http.adornUrl("/smallVientianeTest/rejectSeason/update"),
            method: "post",
            data: {
