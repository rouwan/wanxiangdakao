<template>
  <div class="mod-log">
    ...
    <el-table
      ...
      <el-table-column
       ...
       <el-table-column
        ...
        label="奖品">
        <template slot-scope="scope"> 
          <span v-if="scope.row.gift.length <=0 ">无</span>
          <span v-else>{{rendererGift(scope.row.gift)}}</span>
        </template>
      </el-table-column>
       <el-table-column
        prop="status"
    ...
    <el-pagination
     ...
     <!-- 弹窗, 新增 / 修改 -->
    <AddEmailModules v-if="addVisible" @close="addVisible=false" ref="addModules" @refreshDataList="getDataList"></AddEmailModules>
    <UpdateEmailModules v-if="updateVisible" @close="updateVisible=false" ref="updateModules" @refreshDataList="getDataList"></UpdateEmailModules>
  </div>
</template>

<script>
  import TableTreeColumn from '@/components/table-tree-column'
  import AddEmailModules from './emailAdd'
  import UpdateEmailModules from './emailUpdate' 
    ...
    methods: {
      rendererGift(value){
        ...
        for(var i=0;i<giftArr.length;i++){
          ...
          result += "奖品类型："+giftTypName+"，数量："+giftNum+"。";
        }
      ...
      deleteHandle(id){
         this.$confirm('此操作将永久删除该数据, 是否继续?', '提示', {
          ...
          type: 'warning'
           ...
           this.$http({
              url: this.$http.adornUrl('/smallVientianeTest/email/delete'),
              method: 'get',
      ...
      getDataList () {
        ...
        this.$http({
          url: this.$http.adornUrl('/smallVientianeTest/email/getPageList'),
          method: 'get',
          params: this.$http.adornParams({
