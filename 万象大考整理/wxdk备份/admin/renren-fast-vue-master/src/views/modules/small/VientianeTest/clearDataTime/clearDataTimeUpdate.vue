  ...
  export default {
    ...
    methods: {
      init (id) {
        ...
        this.baseData.id = id || 0;
        this.visible = true;
        this.$nextTick(() => {
          this.$refs['baseData'].resetFields()
          if (this.baseData.id) {
            this.$http({
              url: this.$http.adornUrl(`/smallVientianeTest/clearDataTime/getClearDataTimeById`),
              method: 'get',
              ...
              if (data && data.Tag === 1) {         
                   this.baseData.clearTime = data.Result.clearTime; 
                   this.baseData.type = data.Result.type.toString();                    
              }
            })
          }
        })
      ...
      dataFormSubmit () {
        ...
        this.$refs['baseData'].validate((valid) => {
          ...
          var thiz = this;
          var time = this.baseData.clearTime                   
          try{
            time = this.baseData.clearTime.getFullYear() + "-" + (parseInt(this.baseData.clearTime.getMonth()) + 1) + "-" + this.baseData.clearTime.getDate();     
          }
          catch(e){
            time = this.baseData.clearTime
          }
          this.$http({
            url: this.$http.adornUrl('/smallVientianeTest/clearDataTime/update'),
            method: 'post',
            data: {
              id: this.baseData.id,
              clearTime: time,
