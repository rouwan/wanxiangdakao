    ...
    <el-form :model="subject" :rules="dataRule" ref="subject" label-width="100px">
      ...
      <el-form-item label="图片" prop="image" v-if="subject.modeType==1">
       ...
       <el-form-item label="作者" v-if="this.siciVisible">
        ...
        <el-form-item label="正确答案" prop="answer" v-if="this.answerVisible">
...
export default {
  data() {
    return {
      ...
      dataRule: {
        ...
        audio: [{ required: false, message: "请上传音频", trigger: "blur" }],
  ...
  methods: {
    ...
    dataFormSubmit() {
      ...
      if (this.subject.answer.length <= 0 && this.subject.modeType != 5) {
      ...
      if (isEmpty && this.subject.modeType != 5) {
      ...
      this.$refs["subject"].validate(valid => {
        if (valid) {
          ...
          var thiz = this;
          this.$http({
            url: this.$http.adornUrl("/smallVientianeTest/subject/add"),
            method: "post",
            data: {
              answer: answer,
              image: imageName,
