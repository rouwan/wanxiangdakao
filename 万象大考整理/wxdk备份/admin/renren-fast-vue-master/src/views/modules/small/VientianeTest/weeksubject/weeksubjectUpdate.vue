   ...
   init(id,parentType) {
      ...
      if(parentType > 0){
        this.selChange().then((resolve)=>{
         ...
         this.initTitleSelect().then((resolve)=>{
              this.controllerInit(id);   
          });
      }
    },
    ...
    controllerInit(id) {
      ...
      this.$nextTick(() => {
        ...
        if (this.subject.id) {
          this.$http({
            url: this.$http.adornUrl(`/smallVientianeTest/weekSubject/getSubjectById`),
            method: "get",
            params: this.$http.adornParams({
              id: id
            })
            ...
            if (data && data.Tag === 1) {
              ...
              this.subject.titleId = data.Result.titleId;
    ...
    dataFormSubmit() {
     debugger;
        ...
        if(valid){     
          ...
          var thiz = this;
          this.$http({
            url: this.$http.adornUrl('/smallVientianeTest/weekSubject/update'),
            method: 'post',
            data: {
              id:subjectId,
              answer: answer,
