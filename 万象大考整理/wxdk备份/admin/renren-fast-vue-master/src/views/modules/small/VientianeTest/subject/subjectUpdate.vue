<template>
  <el-dialog
    ...
    <el-form :model="subject" :rules="dataRule" ref="subject"  label-width="100px">
      ...
      <el-form-item label="作者" v-if="this.siciVisible">
            <el-input v-model="subject.author" :value="subject.author" class="title-input" style=" border:0px;"></el-input>
        </el-form-item>
         <el-form-item label="标题" v-if="this.siciVisible">
            <el-input v-model="subject.siciTitle" :value="subject.siciTitle" class="title-input" style=" border:0px;"></el-input>
        </el-form-item>
    ...
    <span slot="footer" class="dialog-footer">
      ...
      <!--审核过的不再修改-->
      <el-button type="primary" v-if="!(this.subjectStatus==1) || this.feedBackSubjectStatus" @click="dataFormSubmit()">确定</el-button>
    </span>
  </el-dialog>
</template>
...
export default {
  ...
  methods: {
    ...
    preSubject() {
      debugger;
      var preSortId = this.subject.sortId - 1;
      this.getSubjectBySortId(preSortId);
    },
    nextSubject() {
      debugger;
      let nextSortId = this.subject.sortId + 1;
      this.getSubjectBySortId(nextSortId);
    },
    getSubjectBySortId(sortId) {
      this.$http({
        url: this.$http.adornUrl(
          "/smallVientianeTest/subject/getSubjectBySortId"
        ),
        method: "get",
            ...
            this.selChange().then(resolve => {
              ...
              for (var i = 0; i < optionObj.length; i++) {
                ...
                if (this.subject.modeType == 4) {
                  //判断题
                  var answerObj = {
                    id: optionLen.toString(),
                    name: optionObj[i].option
                  };
              ...
              this.subject.author = data.Result.author;
              this.subject.siciTitle = data.Result.siciTitle;
    ...
    selChange() {
      ...
      return new Promise((resolve, reject) => {
        this.$http({
          url: this.$http.adornUrl(
            "/smallVientianeTest/subjectType/getSubjectTypeByTypeId"
          ),
          method: "get",
          params: this.$http.adornParams({
    ...
    changeModeType() {
      ...
      if (this.subject.modeType == 4) {
        this.subject.option = [
          { id: 0, option: "对", sortId: 1 },
          { id: 0, option: "错", sortId: 1 }
        ];
        this.rightAnswers = [{ id: "1", name: "对" }, { id: "2", name: "错" }];
      } else if (this.subject.modeType == 5) {
        //成语题默认是选字
        this.subject.option = [];
        this.rightAnswers = [];
        this.answerVisible = false;
      } else if (this.subject.modeType >= 7 && this.subject.modeType <= 10) {
        this.siciVisible = true;
      }
    },
    init(id, parentType,feedBackSubjectStatus = false) {
      debugger;
      if (id == "img[preview]") {
        return;
      }
      ...
      this.feedBackSubjectStatus = feedBackSubjectStatus;
      if (parentType > 0) {
        this.selChange().then(resolve => {
    ...
    controllerInit(id) {
      ...
      this.$nextTick(() => {
        ...
        if (this.subject.id) {
          this.$http({
            url: this.$http.adornUrl(
              `/smallVientianeTest/subject/getSubjectById`
            ),
            method: "get",
            params: this.$http.adornParams({
              id: id
            ...
            if (data && data.Tag === 1) {
              ...
              this.answerVisible = true;
              this.siciVisible = false;
              if (this.subject.modeType >= 7 && this.subject.modeType <= 10) {
                this.siciVisible = true;
              }
              //this.changeModeType();
            }
      ...
      this.reqImageUrl = this.$http.adornUrl(
        `/smallVientianeTest/subject/uploadImage?token=${this.$cookie.get(
          "token"
        )}`
      );
      this.reqAudioUrl = this.$http.adornUrl(
        `/smallVientianeTest/subject/uploadAudio?token=${this.$cookie.get(
          "token"
        )}`
      );
      this.visible = true;
    ...
    dataFormSubmit() {
      ...
      this.$refs["subject"].validate(valid => {
        if (valid) {
          ...
          var siciTitle = subject.siciTitle || "";
          var optionStr = "";
          ...
          this.$http({
            url: this.$http.adornUrl(
              "/smallVientianeTest/subject/updateSubjectById"
            ),
            method: "post",
            data: {
              id: subjectId,
