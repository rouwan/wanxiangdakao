  ...
  <div class="mod-log">
    ...
    <el-table
      ...
      <el-table-column
        ...
        label="类型">
        <template slot-scope="scope"> 
          <span v-if="scope.row.type== 1">战斗力,连胜纪录</span>
          <span v-else>未知</span>
        </template>
      </el-table-column>
       <el-table-column
...
<script>
  ...
  import AddDataConfigModules from './clearDataTimeAdd'
  import UpdateDataConfigModules from './clearDataTimeUpdate' 
  ...
  export default {
    ...
    methods: {
      ...
      deleteHandle(id){
         this.$confirm('此操作将永久删除该数据, 是否继续?', '提示', {
          ...
          type: 'warning'
           ...
           this.$http({
              url: this.$http.adornUrl('/smallVientianeTest/clearDataTime/delete'),
              method: 'get',
      ...
      getDataList () {
        ...
        this.$http({
          url: this.$http.adornUrl('/smallVientianeTest/clearDataTime/getPageList'),
          method: 'get',
          params: this.$http.adornParams({
