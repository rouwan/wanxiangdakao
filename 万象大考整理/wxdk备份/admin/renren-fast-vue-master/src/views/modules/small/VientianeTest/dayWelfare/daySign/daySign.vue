    ...
    <el-table
      ...
      <el-table-column
       ...
       <el-table-column
        ...
        label="第几天">
         <template slot-scope="scope"> 
          <span v-if="scope.row.day== 1">第一天</span>
          <span v-else-if="scope.row.day== 2">第二天</span>
          <span v-else-if="scope.row.day== 3">第三天</span>
          <span v-else-if="scope.row.day== 4">第四天</span>
          <span v-else-if="scope.row.day== 5">第五天</span>
          <span v-else-if="scope.row.day== 6">第六天</span>
          <span v-else-if="scope.row.day== 7">第七天</span>          
        </template>
      </el-table-column> 
      <el-table-column
        prop="name"
       ...
       <el-table-column
        ...
        label="是否显示数量">
         <template slot-scope="scope"> 
          <span v-if="scope.row.isLookNum== 0">否</span>
          <span v-else-if="scope.row.isLookNum== 1">是</span>
        </template>
      </el-table-column> 
       <el-table-column
        prop="description"
...
<script>
  ...
  import AddModules from './daySignAdd'
  import UpdateModules from './daySignUpdate' 
  ...
  export default {
    ...
    methods: {
      ...
      deleteHandle(id){
         this.$confirm('此操作将永久删除该数据, 是否继续?', '提示', {
          ...
          type: 'warning'
           ...
           this.$http({
              url: this.$http.adornUrl('/smallVientianeTest/daySign/delete'),
              method: 'get',
      ...
      getDataList (type) {
        ...
        this.$http({
          url: this.$http.adornUrl('/smallVientianeTest/daySign/getPageList'),
          method: 'get',
          params: this.$http.adornParams({
